upgrade_overlay = document.getElementById('upgrade-overlay');
...
async function showUpgradeBlock(cardType) {
    ...
    const manualData = {
        ...
        genesis: { name: 'Genesis', description: 'Увеличивает колличество монет, которое вы получаете в час', image: 'genesis.png' },
        echeleon: { name: 'Echelon', description: 'Увеличивает колличество монет, которое вы получаете в час', image: 'echelon.png' },
        ledger: { name: 'Ledger', description: 'Увеличивает колличество монет, которое вы получаете в час', image: 'ledger.png' },
        quantum: { name: 'Quantum', description: 'Увеличивает колличество монет, которое вы получаете в час', image: 'quantum.png' },
    ...
    upgrade_overlay.classList.add('active');
...
async function upgradeCard(cardType) {
    ...
    if (response.ok) {
        ...
        upgradeBlock.style.top = '100%';
        upgrade_overlay.classList.remove('active');
...
upgrade_overlay.addEventListener('click', function(){
    const upgradeBlock = document.getElementById('upgrade-block');
    upgradeBlock.style.top = '100%';
    upgrade_overlay.classList.remove('active');
    document.body.style.overflow = 'scroll';
