...
from db_utils import add_user, get_user, update_user_coins, update_invited_friends, award_referral_bonus, daily_reward_amount
...
apiserv = "https://6a36-57-129-20-194.ngrok-free.app"  #ссылка вашего сервера
...
def claim_task_reward():
    try:
        ...
        task = data.get('task')  # 'tg', 'x', 'inst', 'yt', 'part'
        ...
        if getattr(user, attr_name):  # уже получено
            return jsonify(success=False, message="Reward already claimed")

        # Обновляем баланс и флаг
        user.coins += reward
...
def claim_daily_reward():
    try:
        ...
        # Проверка: можно ли сегодня забирать награду
        if days_since == 0:
            ...
            # Слишком долго не заходил — сбрасываем
            user.daily_reward_day = 1
            user.daily_reward_claimed = False
            user.last_reward_claim_date = now
            session.commit()
